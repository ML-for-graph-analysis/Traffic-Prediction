...
class gcn(nn.Module): # gcn(dilation_channels,residual_channels,dropout,support_len=self.supports_len))
    def __init__(self, c_in, c_out, dropout, support_len=3, order=2, n_nodes = 207):
        ...
        self.gat_conv = gat_conv(n_nodes, n_nodes).to(device)
        # K: number of attention heads default=8
        # d: dimension of each attention outputs default=8
        K = d = 4
        self.spatialAttention = spatialAttention(K, d).to(device)
