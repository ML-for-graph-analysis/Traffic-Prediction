...
class nconv(nn.Module):
    ...
    def forward(self, x): 
        ...
        #print('x norm result' + str(x.shape))
        x = torch.einsum('bfnw, bfmw -> bnmw', (x, x)) # S_ij # torch.Size([64, 207, 207, 12])
        #print('x einsum result' + str(x.shape))
        #x = torch.mean(x, dim=0) # torch.Size([207, 207, 12])
        x = torch.mean(x, dim=-1) # torch.Size([207, 207])
        #print('x mean for last' + str(x.shape))
        rowsum = torch.sum(x,dim = 2) #torch.Size([207, 207])
        #print('rowsum' + str(rowsum.shape))
        rowsum = torch.stack([rowsum]*325,dim=-1)
        #print('rowsum' + str(rowsum.shape))
...
class gcn(nn.Module): # gcn(dilation_channels,residual_channels,dropout,support_len=self.supports_len))
    def __init__(self, c_in, c_out, dropout, support_len=3, order=2):
        ...
        device = torch.device("cuda:0") ###여기도 수정 train에 맞춰서
    ...
    def forward(self, x, support):
        ...
        for idx, a in enumerate(support):
            if (idx != 2): # forward, backward
                x1 = torch.einsum('ncvl, nvw -> ncwl',(self.nconv_original(x, a), self.nconv(x)))
                out.append(x1)
                for k in range(2, self.order + 1):
                    x2 = torch.einsum('ncvl, nvw -> ncwl',(self.nconv_original(x1, a), self.nconv(x1)))
                    out.append(x2)
                    # x1 = x2
