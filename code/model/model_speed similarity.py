...
class nconv(nn.Module):
    ...
    def forward(self, x): 
        ...
        A = torch.div(x, rowsum)

        result = torch.einsum('ncvl, vw -> ncwl',(input_x, A))                       
...
class gcn(nn.Module): # gcn(dilation_channels,residual_channels,dropout,support_len=self.supports_len))
    ...
    def forward(self, x, support):
        device = torch.device("cuda:0")
        # k = 0
        # x.shape : torch.Size([64, 32, 207, 12~1])
        out = [x]

        # editted forward, backward -> only forward
        x1 = self.nconv(x).to(device)
        out.append(x1)

        # diffusion step k = 2 일 때만
        for k in range(2, self.order + 1):
            x2 = self.nconv(x1).to(device)
            out.append(x2)

        # editted self-adaptive
        A = support[0] # set forward support to self-adaptive
        x1 = self.nconv_original(x, A) # k = 1 
        out.append(x1)
        ...
        """
        (batch, feature, node, window)
        window: 12, 10, 9, 7, 6, 4, 3, 1
        """
        # h torch.Size([64, 224(32*7), 207, 12~1]) -> [64, 160(32*5), 207, 12~1])
        # linear 1*1 conv
